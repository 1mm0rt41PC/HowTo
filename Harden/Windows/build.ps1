...
if( -not [System.IO.File]::Exists(".\cert\AutoHarden-CA.pvk") -or -not [System.IO.File]::Exists(".\cert\AutoHarden-CA.cer") ){
	makecert -n "CN=AutoHarden-CA" -a sha512 -len 4096 -eku 1.3.6.1.5.5.7.3.3 -r -ss Root -sr localmachine -sy 1mm0rt41PC -sv .\cert\AutoHarden-CA.pvk .\cert\AutoHarden-CA.cer
}
if( -not [System.IO.File]::Exists(".\cert\AutoHarden.pfx") -or -not [System.IO.File]::Exists(".\cert\AutoHarden.cer") ){
	makecert -n "CN=AutoHarden" -a sha512 -len 4096 -eku 1.3.6.1.5.5.7.3.3 -pe -ss MY -iv .\cert\AutoHarden-CA.pvk -ic .\cert\AutoHarden-CA.cer -sy 1mm0rt41PC
	...
	Export-PfxCertificate -Cert $cert -FilePath .\cert\AutoHarden.pfx -Password $password -Force > $null
	Export-Certificate -Cert $cert -FilePath .\cert\AutoHarden.cer -Force > $null
}
$cert = ls Cert:\CurrentUser\My\ | where { $_.Subject.ToString() -eq "CN=AutoHarden" }
...
Get-ChildItem .\src\*.ps1 | foreach {
	...
	cat $_.FullName
	if( [System.IO.File]::Exists($_.FullName.Replace('.ps1','.ask')) ){
		echo '}else{ [System.IO.File]::WriteAllLines($config, "No", (New-Object System.Text.UTF8Encoding $False)); echo 'echo "# ASK... => No :-("' }'
	}
